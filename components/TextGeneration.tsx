...
const TextGeneration: React.FC = () => {
    ...
    const [local, setLocal] = useState<LocalTextGenerationState>(() => ({
        ...initialState,
        model: globalSettings.chatModel as TextModel,
    }));
    ...
    const temperature = globalSettings.temperature; // single source of truth
    ...
    const handleGenerate = async () => {
        ...
        if (isJsonMode) {
            ...
            response = await generateText(prompt, model, fullSystemPrompt, temperature, useWebSearch);
        }
    ...
    const handleRegenerateFromHistory = (entry: TextGenerationHistoryEntry) => {
        ...
        // Use a timeout to ensure state update has rendered before starting generation
                ...
                <div className="flex flex-col gap-4 overflow-y-auto pr-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                           ...
                           <SegmentedControl
                                options={[ {id: 'gemini-2.5-flash', label: 'Flash', tooltip: 'Fast and efficient for everyday tasks.'}, {id: 'gemini-2.5-pro', label: 'Pro', tooltip: 'Most capable model for complex reasoning.'} ]}
                                selected={model}
                                setSelected={(val) => updateSetting('model', val as TextModel)}
                           />
                        </div>
                        <div>
