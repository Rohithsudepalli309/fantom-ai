        ...
        <div className="flex flex-col gap-4 overflow-y-auto pr-1">
          <SectionCard id="api" title="API Key">
            <SettingRow
              ...
              description="We never send your key anywhere except directly to Google’s API from your browser."
              control={(
                <div className="flex flex-col items-end gap-2 w-full max-w-xs">
                  <input
                    ...
                    className="w-full p-2 text-sm rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700"
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => {
                        try {
                          localStorage.setItem('VITE_GEMINI_API_KEY', apiKeyInput);
                          (window as any).VITE_GEMINI_API_KEY = apiKeyInput;
                          setApiKeyInput('');
                          show({ message: 'Gemini API key saved', variant: 'success' });
                    ...
                    <button
                      onClick={() => {
                        try {
                          ...
                          show({ message: 'Gemini API key cleared', variant: 'info' });
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (res.valid) {
                          setKeyStatus('valid');
                          setKeyMsg('API key is valid.');
                          show({ message: 'API key valid', variant: 'success' });
                        } else {
                          setKeyStatus('invalid');
                          setKeyMsg(res.error ?? 'Unknown error');
                          show({ message: 'API key invalid', variant: 'error' });
                        }
                      }}
                  ...
                  {keyStatus !== 'idle' && (
                    <div className={`w-full text-xs p-2 rounded border ${keyStatus === 'valid' ? 'border-green-200 bg-green-50 dark:bg-green-900/30 dark:border-green-800/50 text-green-700 dark:text-green-300' : keyStatus === 'checking' ? 'border-slate-300 bg-slate-50 dark:bg-slate-800/30 dark:border-slate-700 text-slate-600 dark:text-slate-300' : 'border-red-200 bg-red-50 dark:bg-red-900/30 dark:border-red-800/50 text-red-700 dark:text-red-300'}`}>
                      {keyStatus === 'checking' ? 'Verifying…' : keyMsg}
                    </div>
                  )}
          ...
          <SectionCard id="chat" title="Chat">
            ...
            <SettingRow
              title="Default model"
              description="Model used for new chats. You can switch per chat later."
              control={(
                <select
                  value={settings.chatModel}
                  onChange={e => saveSettings({ chatModel: e.target.value as AppSettings['chatModel'] })}
