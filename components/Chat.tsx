...
const Chat: React.FC = () => {
    ...
    const [messages, setMessages] = useState<ChatMessage[]>(() => {
        try {
            const stored = localStorage.getItem('chatHistory_v2');
            return settings.chatHistoryEnabled && stored ? JSON.parse(stored) : [];
        } catch (e) { return []; }
    });
    const [input, setInput] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    const [model, setModel] = useState<TextModel>(settings.chatModel);
    ...
    }, [messages, settings.chatHistoryEnabled]);

    // If history disabled while a conversation exists, clear in-memory messages
    useEffect(() => {
        if (!settings.chatHistoryEnabled && messages.length > 0) {
            setMessages([]);
        }
    ...
    const handleSend = async (messageToSend?: string, historyToUse?: ChatMessage[]) => {
        ...
        try {
            const stream = getChatStream(baseMessages, currentMessage, model, enableRealtimeForThisMessage);
        ...
        <div className="h-full flex flex-col p-2 sm:p-4 bg-white dark:bg-slate-900/50 rounded-2xl shadow-lg">
            <header className="mb-4 flex-shrink-0 pb-3 border-b border-slate-200 dark:border-slate-700/60">
                <div className="flex items-start gap-2 flex-wrap">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Chat</h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Have a conversation with the AI.</p>
                    </div>
                    ...
                    <div className="ml-auto w-full sm:w-auto flex items-center justify-end gap-2 sm:gap-3 flex-wrap">
                        <div
                            className="flex w-full sm:w-auto max-w-xs bg-slate-200 dark:bg-slate-700 rounded-md p-1"
                            role="radiogroup"
                            aria-label="Model selection"
                        >
                            {(['gemini-2.5-flash', 'gemini-2.5-pro'] as const).map(m => (
                                <div key={m} className="relative group flex-1 min-w-[90px]">
                                    <button
                                        onClick={() => {
                                            if (model !== m) {
                                                requestNewChat(() => { setModel(m); setMessages([]); });
                                            }
                                        }}
                                        onKeyDown={(e) => {
                                        ...
                                        aria-label={m.includes('pro') ? 'Select Pro model' : 'Select Flash model'}
                                        className={`w-full text-center text-sm font-medium py-1 rounded transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 ${model === m ? 'bg-white dark:bg-slate-600 shadow text-violet-600 dark:text-violet-300' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600/50'}`}
                                    ...
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-900 text-white text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10">
                                        {m.includes('pro') ? 'Most capable model.' : 'Fast and efficient.'}
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="flex-grow flex flex-col overflow-y-auto p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700/50 space-y-4">
                ...
                {messages.map((msg, index) => (
                    <div key={index} className="animate-msgAppear">
                        {msg.role === 'user' ? (
